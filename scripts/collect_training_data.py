...
def deduplicate(mappings: list[dict]) -> list[dict]:
    """Убрать точные дубли (provide, rpm_name), сохранив первый встретившийся.

    SRPM-уровневая агрегация не применяется намеренно: модель полагается на
    «множественные представления» одного provide в разных подпакетах SRPM —
    это даёт корректное majority при поиске ближайших соседей.
    """
    seen = set()
    unique = []
